...
// Phòng Mua Hàng KHÔNG tự duyệt/từ chối đề xuất của kho — chỉ Quản Lý Kho (CEO/ADMIN).
router.patch('/requests/:id/approve', authMiddleware(['WAREHOUSE_MANAGER', 'CEO', 'ADMIN']), approvePurchaseRequest);
router.patch('/requests/:id/reject', authMiddleware(['WAREHOUSE_MANAGER', 'CEO', 'ADMIN']), rejectPurchaseRequest);
...
// @route   POST /api/v1/purchasing/orders/:id/issue-po
// @desc    Bước 2 sau khi Mua Hàng đã duyệt chọn báo giá (PATCH .../status ->
//          PENDING_PO_DRAFT): lập Phiếu Mua Hàng chính thức — một bản ghi
//          PurchaseOrder MỚI (chứng từ riêng, số riêng) để trình CEO duyệt.
router.post('/orders/:id/issue-po', authMiddleware(['PURCHASING', 'ADMIN']), issuePurchaseOrder);
