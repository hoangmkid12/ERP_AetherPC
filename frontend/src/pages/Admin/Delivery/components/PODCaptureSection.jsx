...
// Chụp ảnh minh chứng (POD) + xác nhận thu tiền/người nhận + trượt xác nhận
// giao thành công. Tách ra từ PODModal để dùng chung ở 2 nơi:
//   - PODModal.jsx (luồng phụ từ OrderDetailSheet): autoStartCamera=true,
...
export default function PODCaptureSection({ order, user, fmt, onConfirm, onReject, autoStartCamera = false }) {
  ...
  // Bật/tắt camera thiết bị đúng lúc vào/rời bước 'camera' — không giữ
  // camera mở ngầm khi đang ở bước form (tốn pin/gây khó chịu quyền truy cập).
  useEffect(() => {
    if (step === 'camera') {
      startCamera();
      return () => { stopCamera(); };
  ...
  const capturePhoto = () => {
    ...
    const ordIdStr = order.orderId || order.id;
  ...
  // ─── Bước form: khung ảnh + thu tiền/người nhận + ghi chú + trượt xác nhận ───
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem', fontSize: '0.82rem' }}>
      {proofPhoto ? (
        ...
        <button
          ...
          style={{
            width: '100%', height: '96px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.3rem',
            border: '2px dashed var(--primary)', borderRadius: 'var(--radius-md)', backgroundColor: 'rgba(37,99,235,0.06)', color: 'var(--primary)',
            fontSize: '0.82rem', fontWeight: 800, cursor: 'pointer'
          }}
        >
          <Camera size={22} />
          Bấm Để Chụp Ảnh Minh Chứng (Bắt Buộc)
        </button>
      )}
      ...
      <div className="delivery-modal-action-bar" style={{ display: 'flex', gap: '0.5rem', alignItems: 'stretch', padding: '0.85rem 1rem 0', background: 'var(--bg-primary)', borderTop: '1px solid var(--border-glass)' }}>
        ...
        {onReject && (
          <button
            ...
            title="Không giao được — báo lỗi nhanh"
            style={{
              flexShrink: 0, width: '52px', height: '52px', borderRadius: '999px',
              backgroundColor: '#fff1f2', color: '#e11d48', border: '1.5px solid #fecdd3',
